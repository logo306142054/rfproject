...
private:
    ...
    {
    public:
        class ICmdHandler *m_pRFCmdhandler;
        ST_CMDINFO *m_stCmdInfo;
        WORD m_wCount;

        _ST_CMDREQUEST(class ICmdHandler *pRFCmdHandler, ST_CMDINFO *pStCmdInfo, WORD wCount)
        {
...
public:
    ...
    void AddCmdHandler(class ICmdHandler *pRFCmdHandler, ST_CMDINFO *pStCmdInfos, WORD wCount)
    {
        ST_CMDREQUEST stCmdRequest(pRFCmdHandler, pStCmdInfos, wCount);
        m_cmdDispatchers.push_back(stCmdRequest);
    }

    void AddRespHandler(class ICmdHandler *pRFCmdHandler, ST_CMDINFO *pStCmdInfos, WORD wCount)
    {
    ...
    //分发平台下发的命令
    void CmdDispatch(WORD wCmdID, CRtnInStream &paraInStream, CRtnOutStream &resultOutStream)
    {
        ...
        while (it != m_respDispatchers.end())
        {
            ...
            for (WORD i = 0; i < (*it).m_wCount; i++)
            {
                ...
                if (pStCmdInfo[i].m_wCmdID == wCmdID)
                {
                    ...
                    CRtnInStream paraInStream(pbyPara, wRespParasLen);
